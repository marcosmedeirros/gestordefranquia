...
function wcJogador(PDO $pdo, string $termo, ?string $ligaDoGrupo = null): string
{
    ...
    if ($s = wcStatsDoJogador($pdo, (int)$p['id'])) {
        // "ÚLTIMA temporada", e não "Temporada N": wcStatsDoJogador devolve o
        // lançamento mais recente que existe, que quase nunca é o da temporada
...
function wcPicks(PDO $pdo, string $termo, ?array $jaResolvido = null, ?string $ligaDoGrupo = null): string
{
    ...
    /* PICK DE DRAFT QUE JÁ PASSOU NÃO É ATIVO.
       A lista vinha com todos os anos, do primeiro ao último, e as picks que
       ainda dá pra negociar ficavam no fim, depois de anos de história que não
       servem pra nada — e quem pede /picks está quase sempre montando troca.
       Quando o draft do ano corrente já aconteceu, nem ele entra: aquela
    ...
    if ($corte > 0) {
        ...
        if (!$picks) {
            return wcNomeDoTime($t) . ' não tem picks futuras — as que ele tinha já foram usadas no draft.';
        }
    }
                               ...
                               string $gatilho = 'comando'): ?string
{
    ...
    try {
        ...
        switch ($cmd) {
            ...
            /* /admin é o índice deles. Fica fora do /ajuda de propósito:
               aquele é o cartaz do grupo dos GMs, e comando que só funciona
               no grupo de admin lá vira pergunta de quem não pode usar. */
            case 'admin':
            case 'ajudaadmin':
            case 'comandosadmin':
                require_once __DIR__ . '/../backend/bot_admin.php';
