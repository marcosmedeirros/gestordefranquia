...
require_once dirname(__DIR__) . '/backend/draft_swaps.php';       // draftAbertoDaLiga()
...
{
    $stats = ['created' => 0, 'renamed' => 0, 'deleted' => 0, 'kept' => 0, 'anos' => $targetYears];
    ...
    /*
     * O ANO DO DRAFT ABERTO NÃO SE RECICLA.
     *
     * A janela começa no ano da temporada corrente. Quando a temporada avança
     * antes de o draft terminar — que é o normal, o draft acontece depois da
     * virada —, o ano que o draft está distribuindo cai fora da janela, e cada
     * pick PRÓPRIA daquele ano era realocada para um ano futuro ou apagada.
     *
     * Só as picks trocadas escapavam, porque a troca marca auto_generated = 0.
     * O efeito na tela era exatamente esse: na Trade Machine o time via as
     * escolhas do draft em andamento que tinha COMPRADO, e não via a própria.
     *
     * Incluir o ano na janela basta: ele passa a ser um alvo ocupado, e o
     * laço de baixo conta a pick como mantida em vez de sobra.
     */
    $anoDoDraft = 0;
    try {
        $draftAberto = draftAbertoDaLiga($pdo, $league);
        $anoDoDraft = (int)($draftAberto['ano'] ?? 0);
        if ($anoDoDraft > 0 && in_array($anoDoDraft, $targetYears, true)) {
            $anoDoDraft = 0;   // já está na janela, nada de especial a fazer
        }
        if ($anoDoDraft > 0) $stats['anos'] = array_merge([$anoDoDraft], $targetYears);
    } catch (Throwable $e) {
        error_log('[seasons/picks_do_draft_aberto] ' . $e->getMessage());
    }
    ...
    foreach ($teams as $team) {
        foreach (['1', '2'] as $round) {
            ...
            /*
             * A pick do ano que o draft está distribuindo sai das duas listas
             * de sobra. Em $autoPicks ela seria candidata a ser realocada pra
             * outro ano; em $autoPicksOutside ela seria APAGADA, por estar
             * fora da janela — que é como ela sumia antes de tudo isso.
             * Ela não é sobra: é a escolha que está prestes a acontecer.
             */
            if ($anoDoDraft > 0) {
                $naoEDoDraft = fn($p) => (int)$p['season_year'] !== $anoDoDraft;
                $autoPicks = array_values(array_filter($autoPicks, $naoEDoDraft));
                $autoPicksOutside = array_values(array_filter($autoPicksOutside, $naoEDoDraft));
            }

            $usedAutoIds = [];

            /*
             * O ANO DO DRAFT ABERTO É INTOCÁVEL, E NÃO CANIBALIZA NINGUÉM.
             *
             * Ele fica FORA de $targetYears de propósito. Quando eu o coloquei
             * lá dentro, ele virou um alvo a preencher — e como é mais antigo
             * que a janela, entrava primeiro no laço: o "candidato" era a pick
             * do ano seguinte, que era REALOCADA pra trás. Pior, $occupiedYears
             * não era limpo no ano de origem, então esse ano continuava
             * marcado como ocupado e ficava vazio para sempre. Foi assim que
             * as picks de 2027 sumiram: viraram picks de 2026.
             *
             * O que ele precisa é só de proteção: a pick daquele ano, se
             * existir, não é sobra pra realocar nem pra apagar; se não
             * existir, nasce uma nova — nunca tirada de outro ano.
             */
            if ($anoDoDraft > 0) {
                if (isset($occupiedYears[$anoDoDraft])) {
                    if ((int)$occupiedYears[$anoDoDraft]['auto_generated'] === 1) {
                        $usedAutoIds[] = (int)$occupiedYears[$anoDoDraft]['id'];
                        $stats['kept']++;
                    }
                } else {
                    $stmtInsert->execute([$team['id'], $team['id'], $anoDoDraft, $round, $seasonId, $team['id']]);
                    $stats['created']++;
                }
            }
            ...
            foreach ($targetYears as $year) {
                ...
                if ($candidate) {
                    ...
                    // O ano DE ONDE ela saiu deixa de estar ocupado. Sem isto,
                    // realocar deixava um buraco permanente: o ano de origem
                    // seguia marcado, o laço o pulava como "mantido", e
                    // ninguém era criado no lugar.
                    unset($occupiedYears[(int)$candidate['season_year']]);
                    $candidate['season_year'] = $year;
    ...
    switch ($action) {
        ...
        case 'run_picks':
            ...
            echo json_encode([
                ...
                'action' => $action,
                'target_years' => $stats['anos'] ?? $targetYears,
                'stats' => $stats
            ]);
            break;
