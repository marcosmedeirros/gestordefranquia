...
function getMaxAutoGeneratedPickYear(PDO $pdo, string $league): int
{
    $stmt = $pdo->prepare("
        SELECT MAX(season_year) as max_year
        FROM picks
        WHERE auto_generated = 1
          AND team_id IN (SELECT id FROM teams WHERE league = ?)
    ");
    $stmt->execute([$league]);
    return (int)($stmt->fetchColumn() ?? 0);
}

function generateAutoPicks(PDO $pdo, array $teams, int $seasonId, int $yearStart, int $yearEnd): int
{
    ...
    foreach ($teams as $team) {
        for ($year = $yearStart; $year <= $yearEnd; $year++) {
            $stmtPick->execute([$team['id'], $team['id'], $year, 1, $seasonId, $team['id']]);
            $stmtPick->execute([$team['id'], $team['id'], $year, 2, $seasonId, $team['id']]);
            $generated += 2;
        }
    }
    ...
    switch ($action) {
        ...
        case 'create_season':
            ...
            if ($requestedYear > 0) {
                ...
                $stmtVerify->execute([$league, $requestedYear]);
                if ((int)$stmtVerify->fetchColumn() > 0) {
                    throw new Exception("Já existe uma temporada registrada para o ano {$requestedYear} na liga {$league}.");
                }
                $year = $requestedYear;
            } else {
            ...
            if (!empty($teams)) {
                if ($seasonNumber === 1) {
                    $pickYears = 5;
                    $startPickYear = $year + 1;
                    $endPickYear = $startPickYear + $pickYears - 1;
                    $generatedPicks = generateAutoPicks($pdo, $teams, $seasonId, $startPickYear, $endPickYear);
                    $picksPerTeam = $pickYears * 2;
                } else {
                    $maxYear = getMaxAutoGeneratedPickYear($pdo, $league);
                    $nextYear = $maxYear > 0 ? ($maxYear + 1) : ($year + 1);
                    $generatedPicks = generateAutoPicks($pdo, $teams, $seasonId, $nextYear, $nextYear);
                    $picksPerTeam = $generatedPicks > 0 ? 2 : 0;
                }
            }
            ...
            echo json_encode([
                'success' => true,
                'season_id' => $seasonId,
                'message' => "Temporada {$seasonNumber} criada! Geradas {$picksPerTeam} picks automáticas por time."
            ]);
            break;
