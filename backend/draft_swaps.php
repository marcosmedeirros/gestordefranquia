...
/**
 * A PARTIR DE QUE ANO AS PICKS DE UM TIME AINDA VALEM.
 *
 * Pick de draft que já passou não é ativo: não dá pra trocar, não dá pra usar,
 * e listá-la só engorda a resposta com história. Pior, ela some no meio das
 * que importam — quem pede /picks está quase sempre montando uma troca.
 *
 * O corte:
 *   - o ano que o draft corrente distribui, quando ele ainda não terminou;
 *   - o ano SEGUINTE, quando já terminou — a classe daquele ano já foi
 *     sorteada, e a pick virou jogador.
 *
 * Sem sessão de draft na sprint ativa não há corte: devolve 0 e quem chama
 * mostra tudo, que é o certo quando o sistema não sabe onde a liga está.
 */
function picksAnoDeCorte(PDO $pdo, string $liga): int
{
    ...
    try {
        /* A sessão mais recente da SPRINT ATIVA. Sprint encerrada é história —
           é a mesma razão de draftAbertoDaLiga olhar só a ativa, e sem isso a
           NEXT cairia na sessão #73 de um sprint fechado, ano 2044. */
        $st = $pdo->prepare("SELECT ds.status, ds.season_id
                               FROM draft_sessions ds
                               JOIN seasons se ON se.id = ds.season_id
                          LEFT JOIN sprints spr ON spr.id = se.sprint_id
                              WHERE se.league = ?
        ...
        $st->execute([$liga]);
        $sessao = $st->fetch(PDO::FETCH_ASSOC);
        if (!$sessao) return $cache[$liga] = 0;

        $ano = draftAnoDasPicks($pdo, (int)$sessao['season_id']);
        if ($ano <= 0) return $cache[$liga] = 0;

        /* `completed` é o único status que significa "essa classe já saiu".
           `setup` e `in_progress` ainda vão distribuir o ano, então a pick
           daquele ano continua sendo um ativo. */
        $terminou = strtolower((string)$sessao['status']) === 'completed';
        return $cache[$liga] = $terminou ? $ano + 1 : $ano;
    } catch (Throwable $e) {
        error_log('[picksAnoDeCorte] ' . $e->getMessage());
        return $cache[$liga] = 0;
    }
}
