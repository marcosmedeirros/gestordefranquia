...
function leilaoSemanaTabela(PDO $pdo): void
{
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS leilao_semana_historico (
        ...
        -- Quem GANHOU o jogo. Vem depois, quando a live acaba e alguém da
        -- organização informa — o leilão decide quem joga, não quem vence.
        vencedor_id INT NULL,
        vencedor_em DATETIME NULL,
        KEY idx_hist (league, temporada),
        KEY idx_hist_time1 (time1_id),
        KEY idx_hist_time2 (time2_id)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");

    // A tabela nasceu sem o vencedor, e CREATE IF NOT EXISTS não mexe em
    // tabela pronta: sem estes ALTERs, o placar só existiria em banco novo.
    if (!$pdo->query("SHOW COLUMNS FROM leilao_semana_historico LIKE 'vencedor_id'")->fetch()) {
        $pdo->exec("ALTER TABLE leilao_semana_historico
                    ADD COLUMN vencedor_id INT NULL,
                    ADD COLUMN vencedor_em DATETIME NULL");
    }
    // Os índices por time: sem eles, o histórico da página do time varre a
    // tabela inteira toda vez que alguém abre um time.
    foreach (['idx_hist_time1' => 'time1_id', 'idx_hist_time2' => 'time2_id'] as $idx => $col) {
        if (!$pdo->query("SHOW INDEX FROM leilao_semana_historico WHERE Key_name = '$idx'")->fetch()) {
            $pdo->exec("ALTER TABLE leilao_semana_historico ADD KEY $idx ($col)");
        }
    }
}

/**
 * Os jogos da semana de um time — os que ele comprou, do mais novo pro
 * mais antigo.
 *
 * O nome do adversário sai do teams quando o time ainda existe, e do que
 * ficou gravado no fechamento quando não existe mais: o histórico é de
 * anos, e time que saiu da liga continua tendo participado daquele jogo.
 */
function leilaoSemanaDoTime(PDO $pdo, int $teamId, int $limite = 30): array
{
    leilaoSemanaTabela($pdo);
    if ($teamId <= 0) return [];
    try {
        $st = $pdo->prepare("
            SELECT h.id, h.league, h.temporada, h.fechado_em, h.vencedor_id,
                   h.time1_id, h.time2_id, h.valor1, h.valor2,
         ...
         LEFT JOIN teams t2 ON t2.id = h.time2_id
             WHERE h.time1_id = ? OR h.time2_id = ?
          ORDER BY h.id DESC
        ...
        foreach ($st->fetchAll(PDO::FETCH_ASSOC) as $h) {
            $souOTime1 = (int)$h['time1_id'] === $teamId;
            $advId     = (int)($souOTime1 ? $h['time2_id'] : $h['time1_id']);
            $venc      = (int)($h['vencedor_id'] ?? 0);
            $out[] = [
                'id'          => (int)$h['id'],
                'league'      => $h['league'],
                'temporada'   => (int)$h['temporada'],
                'quando'      => $h['fechado_em'],
                'pagou'       => (int)($souOTime1 ? $h['valor1'] : $h['valor2']),
                'adv_id'      => $advId,
                'adv_nome'    => (string)($souOTime1 ? $h['time2_nome'] : $h['time1_nome']),
                'adv_logo'    => $souOTime1 ? $h['time2_logo'] : $h['time1_logo'],
                // 'venceu' é null quando o resultado ainda não foi informado
                // — que é diferente de ter perdido.
                'venceu'      => $venc ? ($venc === $teamId) : null,
                'sem_adv'     => $advId === 0,
            ];
        }
        return $out;
    } catch (Throwable $e) {
        error_log('[leilao-semana] do time: ' . $e->getMessage());
        return [];
    }
}

/**
 * Os jogos da semana ENTRE dois times. É o que o /confronto usa.
 *
 * @return array{jogos:array, vitorias_a:int, vitorias_b:int, sem_resultado:int}
 */
function leilaoSemanaEntre(PDO $pdo, int $aId, int $bId): array
{
    leilaoSemanaTabela($pdo);
    $vazio = ['jogos' => [], 'vitorias_a' => 0, 'vitorias_b' => 0, 'sem_resultado' => 0];
    if ($aId <= 0 || $bId <= 0 || $aId === $bId) return $vazio;

    try {
        $st = $pdo->prepare("
            SELECT h.id, h.temporada, h.fechado_em, h.vencedor_id, h.time1_id, h.time2_id
              FROM leilao_semana_historico h
             WHERE (h.time1_id = ? AND h.time2_id = ?)
                OR (h.time1_id = ? AND h.time2_id = ?)
        ...
        $va = $vb = $sem = 0;
        foreach ($jogos as $j) {
            $v = (int)($j['vencedor_id'] ?? 0);
            if     ($v === $aId) $va++;
            elseif ($v === $bId) $vb++;
            else                 $sem++;
        }
        return ['jogos' => $jogos, 'vitorias_a' => $va, 'vitorias_b' => $vb, 'sem_resultado' => $sem];
    } catch (Throwable $e) {
        error_log('[leilao-semana] entre: ' . $e->getMessage());
        return $vazio;
    }
}

/**
 * Os jogos já fechados que ainda não têm resultado informado.
 *
 * É a fila da organização: o leilão decide quem joga, a live acontece, e
 * depois alguém diz quem ganhou. Sem esta lista, o resultado dependeria de
 * alguém lembrar de procurar o jogo certo.
 */
function leilaoSemanaSemResultado(PDO $pdo, string $liga, int $limite = 10): array
{
    leilaoSemanaTabela($pdo);
    try {
        $st = $pdo->prepare("
            SELECT h.id, h.temporada, h.fechado_em, h.time1_id, h.time2_id,
                   COALESCE(t1.name, h.time1_nome) AS time1_nome,
                   COALESCE(t2.name, h.time2_nome) AS time2_nome
              FROM leilao_semana_historico h
         LEFT JOIN teams t1 ON t1.id = h.time1_id
         LEFT JOIN teams t2 ON t2.id = h.time2_id
             WHERE h.league = ? AND h.vencedor_id IS NULL
               -- Jogo sem adversário não tem resultado pra informar: só um
               -- time comprou, e não houve confronto.
               AND h.time1_id IS NOT NULL AND h.time2_id IS NOT NULL
          ORDER BY h.id DESC LIMIT " . max(1, min(50, $limite)));
        $st->execute([strtoupper(trim($liga))]);
        return $st->fetchAll(PDO::FETCH_ASSOC) ?: [];
    } catch (Throwable $e) {
        error_log('[leilao-semana] sem resultado: ' . $e->getMessage());
        return [];
    }
}

/**
 * Grava quem venceu o jogo da semana.
 *
 * O vencedor TEM que ser um dos dois que compraram — não dá pra apontar um
 * time que não estava no jogo, nem por engano de digitação num id.
 */
function leilaoSemanaDefinirVencedor(PDO $pdo, int $historicoId, int $teamId): array
{
    leilaoSemanaTabela($pdo);
    try {
        $st = $pdo->prepare("SELECT time1_id, time2_id FROM leilao_semana_historico WHERE id = ?");
        $st->execute([$historicoId]);
        $h = $st->fetch(PDO::FETCH_ASSOC);
        if (!$h) return ['ok' => false, 'erro' => 'Jogo não encontrado.'];

        // 0 apaga o resultado — serve pra corrigir quem informou errado.
        if ($teamId !== 0 && $teamId !== (int)$h['time1_id'] && $teamId !== (int)$h['time2_id']) {
            return ['ok' => false, 'erro' => 'Esse time não jogou essa partida.'];
        }

        $pdo->prepare("UPDATE leilao_semana_historico
                          SET vencedor_id = ?, vencedor_em = ?
                        WHERE id = ?")
            ->execute([$teamId ?: null, $teamId ? date('Y-m-d H:i:s') : null, $historicoId]);

        return ['ok' => true];
    } catch (Throwable $e) {
        error_log('[leilao-semana] definir vencedor: ' . $e->getMessage());
        return ['ok' => false, 'erro' => 'Não deu pra gravar o resultado.'];
    }
}
