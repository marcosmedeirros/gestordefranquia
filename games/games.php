...
$quemsoueu_basquete_concluiu_hoje = false;
...
try {
    $stmt = $pdo->prepare("SELECT concluido FROM grade_historico WHERE id_usuario = ? AND data_jogo = ? LIMIT 1");
    $stmt->execute([$user_id, $today]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($row) $grade_concluiu_hoje = (bool)$row['concluido'];
} catch (PDOException $e) {}
try {
    $stmt = $pdo->prepare("SELECT concluido FROM boxnba_historico WHERE id_usuario = ? AND data_jogo = ? LIMIT 1");
    $stmt->execute([$user_id, $today]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
...
try {
    $stmt = $pdo->prepare("SELECT resolvido FROM quemsoueu_partidas WHERE id_usuario = ? AND data_jogo = ? LIMIT 1");
    $stmt->execute([$user_id, $today]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
...
try {
    $stmt = $pdo->prepare("SELECT resolvido FROM quemsoueu_basquete WHERE id_usuario = ? AND data_jogo = ? LIMIT 1");
    $stmt->execute([$user_id, $today]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($row) $quemsoueu_basquete_concluiu_hoje = (bool)$row['resolvido'];
} catch (PDOException $e) {}
...
<div class="main">
  ...
  <div class="stats-grid">
    ...
    */ ?>
    ...
    <div class="stat-card">
      <div class="stat-label"><i class="bi bi-question-circle-fill" style="color:#f59e0b"></i>Quem Sou Eu? 🏀 · Hoje</div>
      <div class="stat-value"><?= $quemsoueu_basquete_concluiu_hoje ? '✅' : '—' ?></div>
    </div>
    <?php /* OCULTO PROVISORIAMENTE
    <div class="stat-card">
