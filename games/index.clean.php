...
// Apostas ja encerradas (status='encerrada'), mais recentes primeiro.
// Fechar uma aposta e uma acao manual do admin (dashboard.php, acao=encerrar_evento) --
// nao depende de data_limite/horario, entao nao ha comparacao de timezone aqui.
try {
    try {
...
try {
    ...
    if ($hasStreak) {
        ...
        $stmtTop = $pdo->prepare("SELECT th.id_usuario, th.streak_count, u.nome FROM termo_historico th JOIN games_usuarios u ON u.id = th.id_usuario WHERE th.data_jogo IN (?, ?) ORDER BY th.streak_count DESC, th.data_jogo DESC LIMIT 1");
...
try {
    ...
    if ($hasStreak) {
        ...
        $stmtTop = $pdo->prepare("SELECT mh.id_usuario, mh.streak_count, u.nome FROM memoria_historico mh JOIN games_usuarios u ON u.id = mh.id_usuario WHERE mh.data_jogo IN (?, ?) ORDER BY mh.streak_count DESC, mh.data_jogo DESC LIMIT 1");
        $stmtTop->execute([$today, $yesterday]);
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .bet-closed-tag {
      ...
      color: var(--text-2); text-transform: uppercase; flex-shrink: 0;
    ...
    .bet-opt-mini.won { border-color: rgba(245,158,11,.4); background: rgba(245,158,11,.07); }
    ...
    .bet-opt-mini .mini-pct { font-size: 13px; font-weight: 800; color: var(--text); width: 38px; text-align: right; flex-shrink: 0; }
      ...
      <div class="bet-card">
        <button class="bet-toggle" type="button"
          ...
          <div class="bet-right">
            ...
            <?php else: ?>
              <div class="bet-prize"><img src="lebron.png" style="width:14px;height:14px;object-fit:contain;vertical-align:middle">+75 pts</div>
            <?php endif; ?>
    ...
    ?>
      <div class="bet-card closed">
        <button class="bet-toggle" type="button"
          ...
          <div class="bet-title-wrap">
            <div class="bet-title">
              <span class="bet-closed-tag"><i class="bi bi-lock-fill"></i>Encerrada</span>
