...
<head>
  ...
  <title>Álbum FBA - Coleção Oficial</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .rarity-rara     { border-color: #3b82f6; background: linear-gradient(145deg, #1e3a8a, #2563eb); box-shadow: 0 0 10px rgba(59,130,246,.5); }
    .rarity-epico    { border-color: #a855f7; background: linear-gradient(145deg, #4c1d95, #9333ea); box-shadow: 0 0 15px rgba(168,85,247,.6); }
    .rarity-lendario { border-color: #eab308; background: linear-gradient(145deg, #713f12, #ca8a04); box-shadow: 0 0 20px rgba(234,179,8,.8); animation: pulse-gold 2s infinite; }
    @keyframes pulse-gold { 0% { box-shadow: 0 0 15px rgba(234,179,8,.6); } 50% { box-shadow: 0 0 30px rgba(234,179,8,1); } 100% { box-shadow: 0 0 15px rgba(234,179,8,.6); } }
    ...
    .pack-info-btn {
      ...
      background: rgba(0,0,0,.55); color: #fff; font-weight: 800; z-index: 2;
    ...
    .basketball-court {
      background: #e6915c;
      background-image: repeating-linear-gradient(90deg, transparent, transparent 40px, rgba(0,0,0,.05) 40px, rgba(0,0,0,.05) 80px);
      border: 3px solid #fff; position: relative; width: 100%; max-width: 500px;
      aspect-ratio: 1/1.2; margin: 0 auto; border-radius: 10px; overflow: hidden;
      box-shadow: inset 0 0 50px rgba(0,0,0,.3);
    }
    .court-paint { border: 3px solid #fff; border-bottom: none; position: absolute; bottom: 0; left: 50%; transform: translateX(-50%); width: 160px; height: 200px; background: #cc7a48; }
    .court-3pt { border: 3px solid #fff; border-radius: 50%; position: absolute; bottom: -50px; left: 50%; transform: translateX(-50%); width: 450px; height: 400px; pointer-events: none; }
    .court-slot {
      position: absolute; width: 70px; height: 98px; transform: translate(-50%,-50%);
      border: 2px dashed rgba(255,255,255,.8); border-radius: 8px;
      background: rgba(0,0,0,.3); cursor: pointer; transition: .2s;
      display: flex; align-items: center; justify-content: center;
      color: #fff; font-weight: 700; font-size: 1.5rem; z-index: 10;
    }
    .court-slot:hover { background: rgba(255,255,255,.2); border-style: solid; }
    .court-slot img { width: 100%; height: 100%; object-fit: cover; border-radius: 6px; border: 2px solid #fff; }
...
<body class="min-h-screen flex flex-col">
  ...
  <main class="container mx-auto px-4 py-8 flex-grow" style="max-width:1200px">
    ...
    <section id="section-team" class="hidden text-center">
      ...
      <div class="flex justify-center mb-8">
        <div style="background:var(--panel-2);border:1px solid rgba(234,179,8,.3);border-radius:999px;padding:10px 32px;font-size:14px">
          OVR DO TIME: <span id="team-ovr-display" style="font-size:28px;font-weight:900;color:var(--amber);margin-left:8px">0</span>
        </div>
      </div>
      <div class="mb-6">
        <button type="button" onclick="clearTeam()"
                style="background:var(--panel-2);border:1px solid rgba(234,179,8,.25);border-radius:var(--radius-sm);padding:8px 20px;color:var(--text-2);font-family:var(--font);font-size:13px;font-weight:600;cursor:pointer">
          Limpar escalação
        </button>
      </div>
    ...
    <section id="section-store" class="hidden text-center">
      ...
      <div class="flex flex-wrap justify-center gap-8 mt-10">
        ...
        <div class="flex flex-col items-center">
          <div id="pack-basico" class="pack w-64 h-80 rounded-xl relative flex flex-col justify-center items-center overflow-hidden !border-slate-400"
        ...
        <!-- Premium -->
        <div class="flex flex-col items-center">
          <div id="pack-premium" class="pack w-64 h-80 rounded-xl relative flex flex-col justify-center items-center overflow-hidden !border-blue-400"
               style="background:linear-gradient(135deg,#2563eb,#1e3a8a)" onclick="openPack('premium')">
            <button type="button" class="pack-info-btn" onclick="event.stopPropagation();showPackOdds('premium')">!</button>
            <h3 class="text-3xl font-black italic">PREMIUM</h3>
          </div>
          <button onclick="openPack('premium')"
                  style="margin-top:14px;background:rgba(37,99,235,.2);border:1px solid rgba(59,130,246,.3);border-radius:999px;padding:8px 24px;font-family:var(--font);font-size:13px;font-weight:700;color:#60a5fa;cursor:pointer">
            60
          </button>
        </div>
        <!-- Ultra -->
        <div class="flex flex-col items-center">
          <div id="pack-ultra" class="pack w-64 h-80 rounded-xl relative flex flex-col justify-center items-center overflow-hidden !border-yellow-400"
               style="background:linear-gradient(135deg,#eab308,#713f12)" onclick="openPack('ultra')">
          ...
          <button onclick="openPack('ultra')"
                  style="margin-top:14px;background:#eab308;border:none;border-radius:999px;padding:8px 24px;font-family:var(--font);font-size:13px;font-weight:700;color:#000;cursor:pointer">
            100
          </button>
        </div>
      </div>
    ...
    <section id="section-admin" class="hidden">
      ...
      <p style="color:var(--text-2);font-size:13px;margin-bottom:1.5rem">Cadastrar por time, posição, imagem e raridade.</p>
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div style="background:var(--panel);border:1px solid var(--border-red);border-radius:var(--radius);padding:18px">
          <form id="admin-card-form" class="space-y-3">
            <input id="admin-team"     placeholder="Time"          class="fba-input" required>
            ...
            <input id="admin-img" type="url" placeholder="URL da imagem" class="fba-input" required>
            <button style="width:100%;background:var(--red);color:#fff;border:none;border-radius:var(--radius-sm);padding:9px;font-family:var(--font);font-size:13px;font-weight:700;cursor:pointer">Cadastrar Carta</button>
          </form>
  ...
  <div id="album-card-modal" class="fixed inset-0 modal z-50 hidden flex-col justify-center items-center py-6 px-3">
    <div class="fba-modal-box p-4 w-full max-w-[20rem] sm:max-w-[22rem] max-h-[92vh] overflow-hidden relative">
      ...
      <div style="color:var(--amber);font-size:12px;margin-top:4px" id="album-card-modal-count"></div>
  ...
  <div id="ranking-team-modal" class="fixed inset-0 modal z-50 hidden flex-col justify-center items-center py-6 px-3">
    <div class="fba-modal-box p-4 w-full max-w-5xl max-h-[92vh] overflow-hidden relative">
      ...
      <div class="pr-8 mb-3">
        ...
        <div style="font-size:13px;font-weight:700;color:var(--amber)">OVR: <span id="ranking-team-modal-ovr">0</span></div>
      </div>
