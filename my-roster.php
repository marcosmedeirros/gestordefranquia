...
<script>window.__CAP_TOP_N__ = <?= CAP_TOP_N ?>;</script>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .loyal-badge { background: rgba(6,182,212,.15); color: #06b6d4; border: 1px solid rgba(6,182,212,.35); font-size: 10px; padding: 2px 6px; border-radius: 999px; font-weight: 700; }
        ...
        .loyal-player-card { border-color: rgba(6,182,212,.45) !important; background: rgba(6,182,212,.05) !important; }
        .loyal-player-row { background: rgba(6,182,212,.06) !important; border-left: 3px solid rgba(6,182,212,.45); }
        .loyal-player-li { background: rgba(6,182,212,.05) !important; border-radius: 6px; padding-left: 8px !important; }
        ...
        .roster-table tbody td { padding: 12px 12px; border-bottom: 1px solid var(--border); vertical-align: middle; }
        ...
        .skill-edit-grid select { width:100%; background:var(--panel-2); border:1px solid var(--border); border-radius:8px; padding:6px 2px; color:var(--text); font-size:12px; text-align:center; outline:none; }
                ...
                <form id="form-player">
                    <div class="fgrid">
                        ...
                        <div style="grid-column: span 4; align-self: end;">
                            <button type="submit" class="btn-red" id="btn-add-player" style="width:100%;justify-content:center;padding:12px;">
...
<script>
    ...
    window.__SALARY_CAP__ = <?= ($salaryCapMode && $salCap) ? json_encode(['payroll' => (int)$salCap['payroll'], 'cap_max' => (int)$salCap['cap_max'], 'cap_base' => (int)$salCap['cap_base'], 'cap_flex_total' => (int)$salCap['cap_flex_total'], 'cap_loyalty_total' => (int)$salCap['cap_loyalty_total'], 'cap_floor' => (int)$salCap['cap_floor'], 'status' => $salCap['status'], 'roster' => array_map(fn($r) => ['id' => (int)$r['id'], 'total_salary' => (int)$r['total_salary']], $salCap['roster'])]) : 'null' ?>;
